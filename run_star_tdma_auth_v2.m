...
%   decide_cdma_legit.m (embedded below), decide_csk_legit.m (embedded below).
    ...
    addParameter(p,'log_soft_metrics',false);      % true => log correlator magnitudes for attacker feature
    % in the big addParameter list:
    addParameter(p,'crp_tap','expanded');   % 'raw' | 'expanded'

    parse(p,varargin{:});
    cfg = p.Results;
    
    % ---- NEW: modeling-attack toggles & logs ----
    %cfg.auth_mode = 'controlledPUF';  
    % 'controlledPUF' (default)  — keep your current design (no CRP leakage on-air)
    % 'exposedCRP'               — only logs CRPs to a MATLAB struct for offline LR/SVM (no PHY change)
    
    cfg.log_soft_metrics = false;  
    % when true, we capture per-bit correlator magnitudes (a “soft info” leak) for attacker models
    
    CRP_LOGS = {};         % cell array of structs with fields: node, C (1xn), r (scalar), snr_dB
    SOFT_LOGS = {};        % optional: per-bit correlator |sum(seg.*spread_legit)|
            ...
            for s = 1:cfg.N   % TDMA slots across a "frame"
                ...
                if strcmpi(cfg.scheme,'CDMA') && cfg.K_users_per_slot > 1
                    ...
                    for uu = 1:numel(slot_users)
                        ...
                        % --- NEW: CRP tap (per user in this slot) ---
                        if strcmpi(cfg.auth_mode,'exposedCRP')
                            r_model = get_puf_bits(node, C_eff, 1);   % one PUF bit
                            CRP_LOGS{end+1} = struct('node', iusr, 'C', logical(C_eff(:).'), ...
                                                     'r', logical(r_model), 'snr_dB', SNRdB + node_offsets(iusr));
                        end
                ...
                % --- NEW: CRP tap for modeling (offline) ---
                if strcmpi(cfg.auth_mode,'exposedCRP')
                    if strcmpi(cfg.crp_tap,'raw')
                        % RAW tap: pure APUF bit from this node’s weights (no expansion/PRG)
                        r_model = apuf_bit_from_C(C_eff, nodes(i).W);
                    else
                        % EXPANDED tap: whatever your deployed mapping is
    ...
    results.CRP_LOGS  = CRP_LOGS;   % cell array of {node, C, r, snr_dB}
    results.SOFT_LOGS = SOFT_LOGS;  % optional
    results.auth_mode = cfg.auth_mode;
    results.log_soft_metrics = cfg.log_soft_metrics;
    results.crp_tap         = cfg.crp_tap;   % <— add this
    % --- Plots ---
    figure;
...
function r = apuf_bit_from_C(C_bits, W)
    % C_bits: 1 x n (0/1 or logical). W: (n+1) x 1
