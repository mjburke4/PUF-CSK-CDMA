function results = run_star_tdma_auth(varargin)
...
addParameter(p,'replay_reuse_prob',0.0);
...
% NEW: modeling toggles
addParameter(p,'auth_mode','controlledPUF');   % 'controlledPUF' | 'exposedCRP'
addParameter(p,'log_soft_metrics',false);      % true => log correlator magnitudes
addParameter(p,'crp_tap','expanded');          % 'raw' | 'expanded' (for CRP logger only)
...
% NEW: ensure each node has a per-node APUF weight vector for RAW tap (init once)
for ii = 1:cfg.N
    if ~isfield(nodes(ii),'W') || isempty(nodes(ii).W)
        nodes(ii).W = randn(cfg.challenge_len + 1, 1);
    end
end
...
% Modeling logs
CRP_LOGS  = {};   % {struct('node',i,'C',1xn logical,'r',logical,'snr_dB',val), ...}
SOFT_LOGS = {};
...
for k = 1:K
    ...
    % --- Frames and slots
    for f = 1:cfg.frames
        for s = 1:cfg.N   % TDMA slots per frame
            ...
            if strcmpi(cfg.scheme,'CDMA') && cfg.K_users_per_slot > 1
                ...
                % Optional broadcast challenge/nonce
                C_shared = [];
                ...
                to_store = struct('key',{},'entry',{});

                H = hadamard(cfg.L); used_rows = false(1, cfg.L);
                ...
                for uu = 1:numel(slot_users)
                    ...
                    % NEW: CRP tap (multi-user)
                    % if strcmpi(cfg.auth_mode,'exposedCRP')
                    %     if strcmpi(cfg.crp_tap,'raw')
                    %         r_model = apuf_bit_from_C(C_eff, nodes(iusr).W);
                    %     else
                    %         r_model = get_puf_bits(node, C_eff, 1);
                    %     end
                    %     CRP_LOGS{end+1} = struct('node', iusr, 'C', logical(C_eff(:).'), ...
                    %                              'r', logical(r_model), ...
                    %                              'snr_dB', SNRdB + node_offsets(iusr));
                    % end
                    ...
                    if strcmpi(cfg.auth_mode,'exposedCRP')
                        if strcmpi(cfg.crp_tap,'raw')
                            ...
                            R_exp = get_puf_bits(node, C_eff, cfg.L);
                            CRP_LOGS{end+1} = struct('node', iusr, 'C', logical(C_eff(:).'), ...
                                                     'r', logical(R_exp(1)), ...
                                                     'R', logical(R_exp), ...        % <--- full spread
                                                     'snr_dB', SNRdB + node_offsets(iusr));
                        end
                    end
                    ...
                    % Capture template for replay DB
                ...
                for kk = 1:numel(decode_order)
                    ...
                    if cfg.use_sic
                        rx_for_decision = residual_pre_u;
                    else
                        rx_for_decision = rx_slot;
                    end
            ...
            % Optional broadcast base challenge
            C_shared = [];
            if strcmpi(cfg.challenge_mode,'broadcast')
            ...
            % NEW: CRP tap (single-user)
            % if strcmpi(cfg.auth_mode,'exposedCRP')
            %     if strcmpi(cfg.crp_tap,'raw')
            %         r_model = apuf_bit_from_C(C_eff, nodes(i).W);
            %     else
            %         r_model = get_puf_bits(node, C_eff, 1);
            %     end
            %     CRP_LOGS{end+1} = struct('node', i, 'C', logical(C_eff(:).'), ...
            %                              'r', logical(r_model), 'snr_dB', SNR_node_dB);
            % end

            % --- CRP tap (single-user) ---
            if strcmpi(cfg.auth_mode,'exposedCRP')
                if strcmpi(cfg.crp_tap,'raw')
                    % RAW tap: 1-bit raw APUF
                    r_model = apuf_bit_from_C(C_eff, nodes(i).W);
                    CRP_LOGS{end+1} = struct('node', i, 'C', logical(C_eff(:).'), ...
                                             'r', logical(r_model), ...
                                             'snr_dB', SNR_node_dB);
                else
                    % EXPANDED tap: full L-bit spread R
                    R_exp = get_puf_bits(node, C_eff, cfg.L);  % 1 x L (0/1)
                    CRP_LOGS{end+1} = struct('node', i, 'C', logical(C_eff(:).'), ...
                                             'r', logical(R_exp(1)), ...    % keep a 1-bit too if you like
                                             'R', logical(R_exp), ...       % <--- full spread
                                             'snr_dB', SNR_node_dB);
                end
            end
            ...
            if strcmpi(cfg.scheme,'CDMA')
                ...
                % Unified decision (force)
                [slot_accept, ~] = decide_cdma_legit(obs.rx, (2*get_puf_bits(node, C_eff, cfg.L)-1), ...
                                                     tx_bits, cfg.L, cfg.threshold, cfg.accept_ratio);

                % Soft-metric leak (optional)
                if cfg.log_soft_metrics
                    spread_legit = 2*get_puf_bits(node, C_eff, cfg.L) - 1;
                    Lloc = cfg.L; Nb = cfg.bits_per_slot;
                    soft = zeros(1, Nb);
                    for bb = 1:Nb
                        idx = (bb-1)*Lloc + (1:Lloc);
                        seg = obs.rx(idx);
                        soft(bb) = abs(sum(seg .* spread_legit));
                    end
                    SOFT_LOGS{end+1} = struct('node', i, 'C', logical(C_eff(:).'), ...
                                              'soft', soft, 'snr_dB', SNR_node_dB);
                end

                FAR = 0; FRR = double(~slot_accept);
                ...
                % Store capture
                [tmpl_clean, metaP] = build_csk_template(node, C_eff, tx_bits, cfg.M, cfg.L);
                captureSNRdB = decide_capture_snr(SNR_node_dB, cfg.replay_capture_snr_db, cfg.replay_capture_offset_db);
                tmpl_captured = add_awgn(tmpl_clean, captureSNRdB);
                replay_db(key) = struct('scheme','CSK','node',i,'C',C_eff,'M',cfg.M,'L',cfg.L, ...
                                        'tx_bits',tx_bits,'template',tmpl_captured, ...
                                        'perm_idx',metaP.perm_idx,'sym_idx',metaP.sym_idx);
...
% Export modeling logs & flags
results.CRP_LOGS        = CRP_LOGS;
results.SOFT_LOGS       = SOFT_LOGS;
results.auth_mode       = cfg.auth_mode;
results.log_soft_metrics= cfg.log_soft_metrics;
results.crp_tap         = cfg.crp_tap;
...
function C_eff = mix_challenge_nonce(C_seed, U, target_len)
    if isempty(U), C_eff = C_seed; return; end
...
function ok = preamble_success(SNRdB, len, refSNR)
    margin = SNRdB - refSNR;
    p = 1 ./ (1 + exp(-0.9*margin));
    ok = rand < p^max(1,len/32);
end
...
% ===== Helper: RAW APUF bit for CRP tap =====
function r = apuf_bit_from_C(C_bits, W)
